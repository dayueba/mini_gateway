...
func (worker *MergeWorker) mergeWorkerMain() {
	...
	for {
		...
		case context = <-worker.contextChan:
			...
			// 新建批次, 启动超时自动提交
			if isCreated {
				batch.commitTimer = time.AfterFunc(time.Duration(GlobalConfig.MaxMergerDelay)*time.Millisecond, worker.autoCommit(batch))
			}

			// 批次未满, 继续等待下次提交
			if len(batch.items) < GlobalConfig.MaxMergerBatchSize {
				continue
			}
...
func initMergeWorker(mergeType int) (worker *MergeWorker) {
	worker = &MergeWorker{
		...
		contextChan: make(chan *PushContext, GlobalConfig.MergerChannelSize),
		timeoutChan: make(chan *PushBatch, GlobalConfig.MergerChannelSize),
	}
...
func InitMerger() (err error) {
	...
	merger = &Merger{
		roomWorkers: make([]*MergeWorker, GlobalConfig.MergerWorkerCount),
	}
	for workerIdx = 0; workerIdx < GlobalConfig.MergerWorkerCount; workerIdx++ {
		merger.roomWorkers[workerIdx] = initMergeWorker(PUSH_TYPE_ROOM)
	}
	merger.broadcastWorker = initMergeWorker(PUSH_TYPE_ALL)
...
func (merger *Merger) PushRoom(room string, msg *json.RawMessage) (err error) {
	...
	for _, ch = range []byte(room) {
		workerIdx = (workerIdx + uint32(ch)*33) % uint32(GlobalConfig.MergerWorkerCount)
	}
